    ...
    payload = {
        ...
        "message": [
            ...
            },
            {
                "type": "text",
                "data": {
                    "text": f" 的新提问，编号为 Q{max_id + 1}，归类为 {reply_type}，如需追问，请使"
                    f"用 Q{max_id + 1} 开头的消息进行追问. 问题链接：http://{global_ip}:{http_website_port}/{reply_type}.html#q{max_id+1}"
                }
            }
        ]
    }
            ...
            "message": [
                {
                    "type": "text",
                    "data": {
                        "text": f"[warning] 问题编号 Q{question_id} 不存在，无法添加记录."
                    }
                }
            ]
        }
    ...
    payload = {
        ...
        "message": [
                ...
                }            },
            {
                "type": "text",
                "data": {
                    "text": f" 的消息，已添加到 Q{question_id} 的记录中. 问题链接：http://{global_ip}:{http_website_port}/{qtype}.html#q{question_id}"
                }
            }
        ]
    }
            ...
            "message": [
                {
                    "type": "text",
                    "data": {
                        "text": f"已成功将问题 Q{question_id} 设置为关闭状态."
                    }
                }
            ]
        }
            ...
            "message": [
                {
                    "type": "text",
                    "data": {
                        "text": f"已成功将问题 Q{question_id} 设置为典型问题."
                    }
                }
            ]
        }
            ...
            "message": [
                {
                    "type": "text",
                    "data": {
                        "text": f"已成功将问题 Q{question_id} 设置为无意义问题."
                    }
                }
            ]
        }
